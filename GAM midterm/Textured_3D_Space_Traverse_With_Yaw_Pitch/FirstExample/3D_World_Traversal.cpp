...
{
	// MIDTERM CODE ADDED: rotating cubes
	/* added glm::rotate with alpha as an argument*/
	model_view = glm::scale(model_view, glm::vec3(scale, scale, scale));
	model_view = glm::rotate(model_view, alpha, glm::vec3(scale, scale, scale));
...
void draw_level()
{
	...
	/* MIDTERM CODE ADDED: CREATING SKYBOX */
	/* created a skybox using model_view and applied textures to it */
	model_view = glm::translate(model_view, glm::vec3(model_view[0][1], model_view[0][1], 12.0));
	//model_view = glm::scale(model_view, glm::vec3(1.0, 1.0, 12.0));
	glUniformMatrix4fv(location, 1, GL_FALSE, &model_view[0][0]);
	glBindTexture(GL_TEXTURE_2D, texture[1]);
	glDrawArrays(GL_QUADS, 0, 4);
	...
	for (int i = 0; i < Num_Obstacles; i++)
	{
		model_view = glm::translate(model_view, glm::vec3(obstacle_data[i][0], obstacle_data[i][1], 1.0));
		glUniformMatrix4fv(location, 1, GL_FALSE, &model_view[0][0]);
...
void display(void)
{
	...
	glm::mat4 camera_mat = glm::lookAt(cam_pos, looking_dir_vector, up_vector);
	glUniformMatrix4fv(location, 1, GL_FALSE, &camera_mat[0][0]);
...
void keyboard(unsigned char key, int x, int y)
{
	...
	// MIDTERM CODE ADDED: press G/H to rotate head
	/* may need the assistance of mouse movement */
	if (key == 'g') {
		looking_dir_vector -= side_vector;
	}
	if (key == 'h') {
		looking_dir_vector += side_vector;
	}
...
void idle()
{
	...
	// MIDTERM CODE ADDED: rotating cubes
	float omega = 1;
	timeSinceStart = glutGet(GLUT_ELAPSED_TIME) / 100.f;
