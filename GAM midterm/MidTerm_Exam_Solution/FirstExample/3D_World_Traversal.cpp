...
void init(void)
{
	...
	GLfloat vertices[NumVertices][3] = {
		...
		{ -100.0, -100.0, 10.0 }, //Sky to walk on and a sky
		{ 100.0, -100.0, 10.0 },
		{ 100.0, 100.0, 10.0 },
		{ -100.0, 100.0, 10.0 },
	};
	...
	unsigned char* textureData2 = SOIL_load_image("flower.png", &width2, &height2, 0, SOIL_LOAD_RGB);

	/*GLint width3, height3;
	unsigned char* textureData3 = SOIL_load_image("index.jpg", &width3, &height3, 0, SOIL_LOAD_RGB);*/
...
{
	model_view = glm::rotate(model_view, alpha_rotate, unit_z_vector);
...
void keyboard(unsigned char key, int x, int y)
{
	...
	if (key == 'g')
	{
		up_vector = glm::rotate(up_vector, beta_rotate, forward_vector);
	}
	if (key == 'h')
	{
		up_vector = glm::rotate(up_vector, -beta_rotate, forward_vector);
