...
glm::mat4 model_view;
glm::vec3 wheel_rotate = glm::vec3(0, 1, 0);
glm::vec3 unit_z_vector = glm::vec3(0, 0, 1);	//Assigning a meaningful name to (0,0,1) :-)
glm::vec3 cam_pos = glm::vec3(-5.0f, -5.0f, height);
...
void init(void)
{
	...
	//Setting up parameters for the texture that recently idle into VRAM
...
void drawVehicle(float scaleX, float scaleY, float scaleZ) {
	//model_view = glm::translate(glm::vec3(0, 0, 1));
	model_view = glm::translate(glm::vec3(a, b, 1));
	model_view = glm::scale(model_view, glm::vec3(scaleX, scaleY, scaleZ));
	glUniformMatrix4fv(location, 1, GL_FALSE, &model_view[0][0]);
	...
	// 1st wheel
	model_view = glm::translate(glm::vec3(a + 1, b + 1, 0.2));
	model_view = glm::rotate(model_view, alpha_rotate, wheel_rotate);
	model_view = glm::scale(model_view, glm::vec3(scaleX / 10, scaleY / 5, scaleZ / 5));

	glUniformMatrix4fv(location, 1, GL_FALSE, &model_view[0][0]);
	glDrawArrays(GL_QUADS, 4, 24);

	//// 2nd wheel
	model_view = glm::translate(glm::vec3(a - 1, b + 1.0, 0.2));
	model_view = glm::rotate(model_view, alpha_rotate, wheel_rotate);
	model_view = glm::scale(model_view, glm::vec3(scaleX / 10, scaleY / 5, scaleZ / 5));
	...
	// 3rd wheel
	model_view = glm::translate(glm::vec3(a + 1.0, b - 1, 0.2));
	model_view = glm::rotate(model_view, alpha_rotate, wheel_rotate);
	model_view = glm::scale(model_view, glm::vec3(scaleX / 10, scaleY / 5, scaleZ / 5));
...
void idle()
{
	...
	if (a - cam_pos.x <= 1 && b - cam_pos.y >= -1 && !dead) {
		dead = true;
		cam_pos.z = 0.1;
		up_vector = glm::rotate(up_vector, (float)0.5, forward_vector);
	}
	a += 0.001;
	//b -= 0.001;
	glutPostRedisplay();
