...
//void shootBullet(float poseX, float poseY) {
//	model_view = glm::translate(glm::vec3(poseX, poseY, height));
//	int a = forward_vector.x;
//	int b = forward_vector.y;
//	for (int i = 0; i < 100; i++) {
//		model_view = glm::translate(model_view, glm::vec3(a, b, height));
//		a += 0.01;
//		b += 0.01;
//	}
...
void shootBullet(float scaleX, float scaleY, float scaleZ) {
	//model_view = glm::translate(glm::vec3(0, 0, 1));
	model_view = glm::translate(glm::vec3(a, b, height));
...
void keyboard(unsigned char key, int x, int y)
{
	...
	if (key == 'f') {
		...
		//shootBullet(cam_pos.x, cam_pos.y);
		shootBullet(2.0, 2.0, 2.0);
	}
	//}
}
...
void idle()
{
	...
	a += forward_vector.x / 1000;
	b += forward_vector.y / 1000;
