...
// ADDED CODE: Shoot in forward_direction
void shoot() {
	Obj b;
	b.x_pos = cam_pos.x;
	b.y_pos = cam_pos.y;
	b.isBulllet = TRUE;
	b.direction = glm::vec2(forward_vector.x, forward_vector.y);

	collection.push_back(b);
	//model_view = glm::mat4(1.0);
	/*glm::mat4 model_view2 = glm::mat4(1.0);
	model_view2 = glm::translate(model_view2, glm::vec3(forward_vector.x, forward_vector.y, 0));
...
// CODE ADDED: initialize all objects
void initialize() {
	for (int i = 0; i < Num_Obstacles; i++)
	{
		Obj v;
...
// CODE ADDED: Draw an object
void drawObj(float scale, glm::vec2 direction, bool isBullet) {
	//Select the second texture (apple.png) when drawing the second geometry (cube)
	//glBindTexture(GL_TEXTURE_2D, texture[1]);
...
void draw_level()
{
	...
	for (int i = 0; i < collection.size(); i++)
	{
		...
		// CODE ADDED: Only move if isBulllet
		if (v.isBulllet) {
		...
		drawObj(obstacle_data[i][2], moving_dir, v.isBulllet);
	...
	/*for (int i = 0; i < collection.size(); i++) {
		...
		model_view = glm::translate(model_view, glm::vec3(v.x_pos, v.y_pos, 0.0));
		glUniformMatrix4fv(location, 1, GL_FALSE, &model_view[0][0]);

		drawObj(obstacle_data[i][2]);
